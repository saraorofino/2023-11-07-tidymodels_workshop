...
Try it with a new formula above, can you find a model that improves accuracy?

Metrics: we can use metrics from the `yardstick` package to test accuracy and the Receiver Operating Characteristic curve...
```{r}
yardstick::accuracy(surv_test_predict, truth = survived, estimate = .pred_class)
```
...
Now we've specified how the data should be used inside a model, let's apply a model to it!  We'll use a binary logistic regression, which predicts the probability (technically, log odds) of outcome A vs. outcome B based on a linear combination of predictors.
